...
function setsidesegs(sideindex, sidecode) {
    ...
    sidecodeseg.segs = [];
    let x11 = side.x11, y11 = side.y11, x12 = side.x12, y12 = side.y12, x21 = side.x21, y21 = side.y21, x22 = side.x22, y22 = side.y22;
    let sx1 = 0, sy1 = 0, sgx1 = 0, sgy1 = 0, sgx2 = 0, sgy2 = 0, angle = 0, sideshapesplit = 0;
    ...
    for (let j = 0; j < sidecodeshapes.length; j++) {
        ...
        let typeid = sideshape.typeid, sh = sideshape.h;        
        sgx2 = sideshape.x11, sgy2 = sideshape.y11;              
        setsidecodeseg(sx1, sy1, sgx1, sgy1, sgx2, sgy2, sh, typeid, sidecodeseg);
        sgx1 = sideshape.x12, sgy1 = sideshape.y12;       
    }    
...
function onshapeside() {
    ...
    if (mode == 'shape') {
        ...
        if (drawmode == 3) {
            ...
            onsetsidesplash(index, code, 1, splheight, countersidemode, source);
        } else if (drawmode == 5) {
            onsetsideedge(index, code, edgetype, edgetypeid, source);
        } else if (drawmode == 8) {
            onsetsideapron(index, code, 1, edgetype, edgetypeid, splheight, countersidemode, source);
        }
    }
}
...
function editshapeside(index, code, prevval, newval) {
    if (!validatesidechange34(index, code, prevval, newval)) { return; }  
    let side = shape.sides[index];
    let w = roundToTwo(side.w), w1 = roundToTwo(side.w1), w2 = roundToTwo(side.w2), h = side.h, dw = 0;
    let changetype = 1;
    if (code == 1 || code == 2) {
        if (prevval == w1) {
            dw = roundToTwo(newval - side.w1);
            if (isNaN(dw)) { return; }
            side.w1 = newval, side.w2 += dw, side.w += dw; changetype = 1;
        }
        else {
            dw = roundToTwo(newval - side.w2);
            if (isNaN(dw)) { return; }
            side.w2 = newval; side.w1 += dw, side.w += dw; changetype = 2;
        }
...
function validatesidechange(index, code, prevval, newval) {
   ...
   // if (prevval == w1) { dw = newval - w1, w1 = newval, w2 += dw; } else { dw = newval - w2, w2 = newval; w1 += dw; }
    ...
    //console.log(index, code, prevval, newval, minwh, isvalid);
...
function validatesidechange34(index, code, prevval, newval) {
    let source = 'validatesidechange34';
    let isvalid = true;
    let side = shape.sides[index], side1 = shape.sides[index - 1], side2 = shape.sides[index + 1], angle = 0, angle1 = 0, angle2 = 0;
    let w = roundToTwo(side.w), w1 = roundToTwo(side.w1), w2 = roundToTwo(side.w2), h = side.h, dw = 0, h2 = 0, dh = 24;
    // if (prevval == w1) { dw = newval - w1, w1 = newval, w2 += dw; } else { dw = newval - w2, w2 = newval; w1 += dw; }   
    if (code == 1 || code == 2) {       
        if (prevval == w1) { if (w1 >= w2) { if (newval < minwh) { isvalid = false; } else { dw = roundToTwo(w2 + (newval - w1)); if (dw <= 0 || isNaN(dw)) { isvalid = false; } }}           
        } else if (prevval == w2) { if (newval <= 0) { isvalid = false; }}
    } else {
        if (newval < minwh) { isvalid = false; }
    }
    if (isvalid) { isvalid = validatesidechangecorners32(index, code, prevval, newval, source); }
    ...
    //console.log(index, code, prevval, newval, minwh, w1, w2, h,dw, isvalid);
    return isvalid;   
}
function validatesidechangecorners32(sideindex, sidecode, prevval, newval, source) {
    ...
    if (sidecode == 0) { //corner index =0, 1
        ...
        if (sideindex == shape.sides.length - 1) { //corner index =0, 2
            ...
            if (side.corners[1].type != 0) { whr1 = applymminchfact(side.corners[1].w, 2, 2, source); }
            if (shape.sides[sideindex + 1].corners[3] != undefined) { if (shape.sides[sideindex + 1].corners[3].type != 0) { whr2 = applymminchfact(shape.sides[sideindex + 1].corners[3].h, 2, 2, source); } }            
            if ((whr1 + whr2) > newval) { isvalid = false; }
        }
...
function shapesides(source) {
    resetshapecorners34(source);
    resetshapesplashs34(source);
...
function drawshapesides(index, source) {
    if (shape.sides != null) {
        for (let j = 0; j < shape.sides.length; j++) {
            if (shape.sides[j].spsideindex == undefined) { shape.sides[j].spsideindex = shape.sides[j].index;}
            drawsidecorners(j, source + ":drawshapesides");            
            drawsideshapes(j);
            drawsidesplashs(j, source);
            if (shape.mode != 'slabpart') { drawsidecutouts(j); }
            drawsideseams(j);
    ...
    if (isseg) {
       // sidesh = 30;
        for (let i = 0; i < sidecodeseg.segs.length; i++) {
            ...
            //console.log("sindex:" + shape.index, "lnindex:" + i, "index:" + index, "code:" + sidecode, "sx1:" + sx1, "sy1:" + sy1, "sx2:" + sx2, "sy2:" + sy2);
            ...
            //if (stypeid == 4 || stypeid == 5 || stypeid == 6) {
               // if (sh > sidesh) { sidesh = sh + 15; }
                if (sw > 0) {                    
                    ...
                    if (innersizemode == 1) { drawsidemeasureline32(i, index, sidecode, sx1, sy1, sx2, sy2, sw, sgdh, sw, angle, false, isseg); }                 
                  //  drawsidemarkertext32(i, index, sidecode, mtx, mty, sw, angle, false);
                }
            //}
        }
    } else {
       // console.log("sindex:" + shape.index, "lnindex:" + sidecodeseg.segs.length, "index:" + index, "code:" + sidecode, "sx1:" + x1, "sy1:" + y1, "sx2:" + x2, "sy2:" + y2);
        drawsideline32(sidecodeseg.segs.length, index, sidecode, x1, y1, x2, y2, edgetype, stwidth);
    }  
    drawsidemeasureline32(sidecodeseg.segs.length, index, sidecode, x1, y1, x2, y2, sidesw, sidesh, sidesw, angle, true, isseg);
    ...
    if (index > 0 && (index < shape.sides.length - 1)) {//ushape and above  shapes center measure lines
       ...
       // console.log(index, sidecode, mtx, mty, h, angle, editmode);
        if (editmode == 1) {
            ...
            drawsidemarkertext32(sidecodeseg.segs.length + 1, index, sidecode, mtx, mty, h, angle, true);
        }       
        //drawsidemeasureline32(sidecodeseg.segs.length + 2, index, sidecode, x1, y1, x2, y2, h, 0, angle, true);
    }
}
function drawsidemeasureline32(lnindex, index, sidecode, x1, y1, x2, y2, sw, dh, mtext, angle, isedit, isseg) {
    if (mtext < 2) { return;}
    if (shape.mode == 'slabpart') { return; }
    if (dh != 0) {
        if (sidecode == 0 || sidecode == 2) { dh = -dh; }
        let npt = offsetline(x1, y1, x2, y2, dh);
        x1 = npt.dx1, y1 = npt.dy1, x2 = npt.dx2, y2 = npt.dy2;
    }
    //console.log(lnindex, index, sidecode, x1, y1, x2, y2, sw, dh, mtext, angle, isedit, isseg);
    let mtx = Math.round((x1 + x2) / 2), mty = Math.round((y1 + y2) / 2);
    if (innersizemode == 0) { drawsidemarkerline32(lnindex, index, sidecode, x1, y1, x2, y2); }
    else if (!(isedit && isseg)) {
...
function drawshapesidepath32(index, spath) {   
    let sindex = shape.index, smode = shape.mode, partsrno = shape.partsrno, areaid = shape.areaid, materialid = shape.materialid, spindex = shape.spindex, spsideindex = shape.spsideindex, spseamindex = shape.spseamindex,
        startsideindex = shape.startsideindex, startseamindex = shape.startseamindex, endsideindex = shape.endsideindex, endseamindex = shape.endseamindex, sidelen = shape.sides.length;
    //console.log("sindex:" + sindex, "spindex:" + spindex, "spseamindex:" + spseamindex, "spsideindex:" + spsideindex, "partsrno:" + partsrno, "areaid:" + areaid, "mode:" + mode);   
    ...
    sidepathg.attr("index", index).attr("sindex", sindex).attr("partsrno", partsrno).attr("areaid", areaid).attr("materialid", materialid).attr("smode", smode).attr("sidelen", sidelen)
        .attr("scolor", shape.color).attr("d", spath).attr("style", "fill:" + shape.color).style("fill-opacity", "0.2").on("click", onshape);
    if (shape.mode == 'slabpart') {
        sidepathg.attr("spindex", spindex).attr("spseamindex", spseamindex).attr("spsideindex", spsideindex)
            ...
            .on("mouseover", showdgslabpartshape34).on("mouseout", hidedgslabpartshape34);
    }
    if (shape.mode == 'shape' || shape.mode == 'slabpart') { sidepathg.on("contextmenu", onshapecontextmenu); }
}
function drawsideline32(lnindex, index, code, x1, y1, x2, y2, edgetype, stwidth) {
    let sindex = shape.index, smode = shape.mode, partsrno = shape.partsrno, areaid = shape.areaid, spindex = shape.spindex, areaindex = shape.areaindex;
    let etypes = [];
    if (edgetype != '' && edgetype != null) { etypes = edgetype.split(':'); edgetype = etypes[0]; }
    let eclass = sideedgeclass(edgetype);
    let sidecodegindex = shape.mode + "sidecodeg" + sindex + index + "" + code;
    let sidecodeg = sidesg.select("#" + sidecodegindex);
    let sidelineindex = areaid + "" + partsrno + "" + sindex + "" + index + "" + code + "" + lnindex;
    //console.log("sindex:" + sindex, "lnindex:" + lnindex, "index:" + index, "code:" + code, "x1:" + x1, "y1:" + y1, "x2:" + x2, "y2:" + y2, "edgetype:" + edgetype, "stwidth:" + stwidth);
    let spath = "M" + x1 + "," + y1 + "l" + (x2 - x1) + "," + (y2 - y1);
    if (!sidecodeg.empty()) {
        let sideline = sidecodeg.select("#" + smode + "sideline" + sidelineindex);          
        if (sideline.empty()) { sideline = sidecodeg.append("path").attr("id",  smode + "sideline" + sidelineindex); }
        sideline.attr("d", spath).attr("class", eclass).attr("index", index).attr("code", code).attr("edgetype", edgetype).attr("sindex", sindex).attr("spindex", spindex)
        ...
        if (shape.mode == 'shape') {
            ...
            let shapebandline = sidecodeg.select("#" + smode + "sideband" + sidelineindex);
            if (shapebandline.empty()) { shapebandline = sidecodeg.append("line").attr("id",  smode + "sideband" + sidelineindex).on("click", onshapeside); }
        ...
        if (index == 0) {
            ...
            if (lw > 36 && code == 2) { drawlabel = true;}
            if (drawlabel) {
                let sidecodetext = sidecodeg.select("#sidecodetext" + sidelineindex);
                let sidetext = 'A' + (areaindex) + ':P' + (shape.partsrno);
                if (spindex > 0) { sidetext = sidetext + "." + (spindex + 1); }
                if (sidecodetext.empty()) {
                    sidecodetext = sidecodeg.append("text").append("textPath").attr("id", "sidecodetext" + sidelineindex)
                    .attr("xlink:href", "#" + smode + "sideline" + sidelineindex).attr("startOffset", 5).attr("style", "fill:grey;font-size:10px;").text(sidetext);
                }
            }
        }        
    }
}
function drawsideline33(lnindex, index, code, spath, edgetype, drawlabel) {
    let sindex = shape.index, smode = shape.mode, partsrno = shape.partsrno, areaid = shape.areaid, spindex = shape.spindex, areaindex = shape.areaindex;
    let eclass = sideedgeclass(edgetype);
    let sidecodegindex = shape.mode + "sidecodeg" + sindex + index + "" + code;
    let sidecodeg = sidesg.select("#" + sidecodegindex);
    ...
    if (!sidecodeg.empty()) {
        ...
        if (drawlabel) {
            let sidecodetext = sidecodeg.select("#sidecodetext" + sidelineindex);
            let sidetext = 'A' + (areaindex) + ':P' + (partsrno);
            if (spindex > 0) { sidetext = sidetext + "." + (spindex + 1); }
            if (sidecodetext.empty()) {
                sidecodetext = sidecodeg.append("text").append("textPath").attr("id", "sidecodetextpath" + sidelineindex)
                .attr("xlink:href", "#" + smode + "sideline" + sidelineindex).attr("startOffset", 5).attr("style", "fill:grey;font-size:10px;").text(sidetext);
            }
        }
    }
...
function drawsidemarkerline32(lnindex, index, code, x1, y1, x2, y2) {
    ...
    if (markerline.empty()) { markerline = sidecodeg.append("path").attr("id", "mline" + mindex); }    
    let arrowstart = shape.mode == 'print' ||  shape.mode == 'shapedg' ? "url(#printtriangle-start)" : "url(#triangle-start)";
    let arrowend = shape.mode == 'print' ||  shape.mode == 'shapedg' ? "url(#printtriangle-end)" : "url(#triangle-end)";
    ...
    markerline.attr("d", spath).attr("smode", shape.mode)
            .attr("style", "stroke:lightgray;stroke-dasharray:4px;opacity: 0.5;").attr("marker-start", arrowstart).attr("marker-end", arrowend);
}
function drawsidemarkertext32(lnindex, index, code, mtx, mty, mtext, angle, isedit) {
    ...
    let etype = sideedgetext1(index, code);
    let sidecodegindex = shape.mode + "sidecodeg" + shape.index + index + "" + code;
    ...
    if (!sidecodeg.empty()) {
        ...
        markertext.attr("x", mtx).attr("y", mty).attr("lnindex", lnindex).attr("index", index).attr("code", code).attr("sindex", sindex).attr("spindex", spindex)
            .attr("partsrno", partsrno).attr("areaid", areaid).attr("smode", smode);
        let mtextspan = markertext.attr("x", mtx).attr("y", mty).append("tspan").attr("id", "mtextspan1" + mindex).attr("text-anchor", "middle").attr("alignment-baseline", "ideographic");
        if (etype.length > 0) {
            markertext.append("tspan").attr("id", "mtextspan2" + mindex)
                .attr("style", "fill:goldenrod;text-anchor:middle;alignment-baseline:ideographic;font-size:10px;").text(" | " + etype);
        }
        if (smode == 'shape' && isedit == true) {
            mtextspan.attr("style", "fill:black;text-anchor:middle;alignment-baseline:ideographic;font-size:10px;").text(mtext); markertext.on("click", onsidemarkertext);
        }
        else {
            mtextspan.attr("style", "fill:grey;text-anchor:middle;alignment-baseline:ideographic;font-size:10px;").text(mtext);
        }
...
function onsidemarkertext() {
    let source = "onsidemarkertext";
    if (bdraw) { onendshapedraw(source); }
    if (editmode == 1) { editmode = 0; drawshapesides(shape.index, source); } //check drawshapesides() called twice??
    ...
    setdrawshape31(sindex, partsrno, areaid, mode, source);
    editmode = 1;
    drawshapesides(sindex, source);
    selectsideinputcontrol(sindex, index, code, source);
}
