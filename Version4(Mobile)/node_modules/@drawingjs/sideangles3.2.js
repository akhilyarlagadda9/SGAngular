function calcshapeangle(index) {
    ...
    angseg = (angseg - Math.floor(angseg)) > 0.5 ? Math.round(angseg) : Math.floor(angseg);
    angle = angseg * 22.5;
    //console.log(shape.index, index, angle1, angle2, angle);
    return angle;
}
function onshapeangle() {
    let source = 'onshapeangle';
    ...
    if (smode == 'slabpart') {
        onsetslabpartshape31(sindex, spindex, startseamindex, startsideindex, endseamindex, endsideindex, partsrno, areaid, smode, source);        
        if (drawmode == 7 && seamtype == 2) {
            //console.log(sindex, spindex, shape.mode, startseamindex, startsideindex, endseamindex, endsideindex, partsrno, areaid, smode, drawmode, seamtype);
            onsetsidecornerseam31(Math.round(m[0]), Math.round(m[1]), -1, index, sindex, seamtype, source); setdrawmode(0);
        }       
    } else {
        setdrawshape31(sindex, partsrno, areaid, smode, source);
        if (smode == 'shape') { drawcorneranglehandles(index); }
    }    
}
...
function drawshapeangle(index) {
    let sindex = shape.index, smode = shape.mode, partsrno = shape.partsrno, areaid = shape.areaid, materialid = shape.materialid, spindex = shape.spindex,
        spsideindex = shape.spsideindex, spseamindex = shape.spseamindex, startsideindex = shape.startsideindex, startseamindex = shape.startseamindex, endsideindex = shape.endsideindex,
        endseamindex = shape.endseamindex;
    let side = shape.sides[index];
    let cx = side.x2, cy = side.y2, cr = 18, mtx = 0, mty = 0;
    let sx1 = side.x12, sy1 = side.y12, sx2 = side.x22, sy2 = side.y22, sh1 = side.h, sh2 = shape.sides[index + 1].h, sh = Math.min(sh1, sh2, cr);
    cx = (sx1 + sx2) / 2, cy = (sy1 + sy2) / 2;
    if (cr > sh) { cr = sh / 2; }   
    let cangle = calcshapeangle(index);
    let caclass = 'shapesplash';
    let areashapeangleindex = areaid + "" + sindex + "" + index;
    let cangletext = shapepathg.select("#" + smode + "cangletext" + areashapeangleindex);
    if (cangletext.empty()) { cangletext = shapepathg.append("text").attr("id", smode + "cangletext" + areashapeangleindex); }
    cangletext.attr("x", cx).attr("y", cy).text(cangle).attr("index", index).attr("sindex", sindex).attr("spindex", spindex).attr("partsrno", partsrno).attr("areaid", areaid).attr("smode", smode)
    .attr("spseamindex", spseamindex).attr("spsideindex", spsideindex).attr("startseamindex", startseamindex).attr("startsideindex", startsideindex).attr("endseamindex", endseamindex).attr("endsideindex", endsideindex)
    .attr("style", "fill:black;font-size:10px;cursor:pointer;");
    let canglecircle = shapepathg.select("#" + smode + "canglecircle" + areashapeangleindex);
    if (canglecircle.empty()) { canglecircle = shapepathg.append("circle").attr("id", smode + "canglecircle" + areashapeangleindex); }
    canglecircle.attr("cx", cx).attr("cy", cy).attr("r", cr).attr("class", caclass).attr("index", index).attr("sindex", sindex).attr("spindex", spindex).attr("partsrno", partsrno).attr("areaid", areaid).attr("smode", smode)
    .attr("spsideindex", spsideindex).attr("startseamindex", startseamindex).attr("startsideindex", startsideindex).attr("endseamindex", endseamindex).attr("endsideindex", endsideindex).attr("style", "cursor:pointer;");
    if (shape.mode == 'shape' || shape.mode == 'slabpart') {canglecircle.on("click", onshapeangle); cangletext.on("click", onshapeangle);}
    if (cangletext.node() != null) {
        let mtextlen = cangletext.node().getComputedTextLength();
        cx -= Math.round(mtextlen / 2), cy += Math.round(cr / 4);
    }
    cangletext.attr("x", cx).attr("y", cy);
    if (shape.angle != 0) {       
        let gnode = canglecircle.node();
        if (gnode != null) {
            let sx = gnode.getBBox().x + gnode.getBBox().width / 2; sy = gnode.getBBox().y + gnode.getBBox().height / 2;
            let npt = [sx.toFixed(), sy.toFixed()];
            cangletext.attr("transform", "rotate(" + -shape.angle + "," + npt + ")");
        }
    }   
}
