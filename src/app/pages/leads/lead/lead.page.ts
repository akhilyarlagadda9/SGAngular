...
export class LeadPage implements OnInit {
  ...
  ActionRenderEvent(evnt) {
    ...
    let strCustName = "";
    strCustName = event.CustName;
    console.log(event);
      this.leadService.GetLeadInfo(event.LeadID).subscribe(data => {
        strCustName = data.CustomerContacts[0].Name; // get it from Service
        var htmlstring = '';
  ...
  ActionEditActivity(ev) {
    ...
    this.objEditActivity = {
      ID: ev.event._def.extendedProps.ID, ActTypeID: ev.event._def.extendedProps.ActTypeID, StartDate: sDate, EndDate: eDate,
